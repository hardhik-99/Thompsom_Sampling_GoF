...
class ThomSamp_MeanDiff():
    def __init__(self, n=n, N=N, mean_ref=mean_ref, num_arms=num_arms):
        self.n = n
        self.N = N
    ...
    def change_detection(self, i):
        if (self.count[i] > (self.N + self.n)):
            test = np.mean(np.ravel(self.cache[i][-self.n:]))
            estimate = np.mean(np.ravel(self.cache[i][(-self.n-self.N):(-self.n)]))
            mean_diff = abs(test - estimate)
            if(mean_diff > self.mean_ref):
                return True
            else:
                return False
        else:
